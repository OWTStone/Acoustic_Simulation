...
#print(float_data)
  ...
  if file_count == 0:
    ...
    xData[:,file_count] = xData[:,file_count]/max(xData[:,file_count]) # try normailzing data
    #xData[:][file_count] = np.column_stack(xData, yf[:N//2],1)
  audio_type = path_file[1]
  drone_flagg = int.from_bytes(drone_flag)
  if file_count < 80: #120:#drone_flagg:
    yData[file_count] = 1 #np.append(yData,format(1,'b'))
...
def make_model(input_shape):
    ...
    conv1 = keras.layers.Conv1D(filters=1024, kernel_size=3, padding="same")(input_layer)
